/*
 * sim-802-15-4-static.cc
 *
 * Emulates IEEE 802.15.4 characteristics using 802.11b WiFi with:
 *   - Reduced Tx power (~9 dBm → ~30 m range, vs 802.15.4 typical 10–30 m)
 *   - Smaller packet size (80 bytes ≈ 802.15.4 max useful payload)
 *   - Smaller deployment area (50 m × areaFactor)
 *
 * NOTE: ns-3.39's SixLowPanNetDevice always passes IPv6 protocol numbers
 * to the upper layer, making IPv4+AODV over LrWpan+SixLowPan impossible.
 * This WiFi-based emulation preserves the key 802.15.4 characteristics
 * (short range, small MTU) while keeping FF-AODV fully functional.
 */
...
int main(int argc, char *argv[])
{
    ...
    double      txRange    = 50.0;    // 802.15.4 shorter range (m) — area calc only
    ...
    // ── Wi-Fi 802.11b — configured to emulate 802.15.4 characteristics ───
    // TxPower 9 dBm → effective range ≈ 30 m (802.15.4-like short range).
    WifiHelper wifi;
    ...
    // Reduced Tx power to emulate 802.15.4 short-range characteristic
    phy.Set("TxPowerStart", DoubleValue(9.0));
    phy.Set("TxPowerEnd",   DoubleValue(9.0));
    ...
    aodv.Set("Gamma",         DoubleValue(0.0));   // velocity disabled for static
