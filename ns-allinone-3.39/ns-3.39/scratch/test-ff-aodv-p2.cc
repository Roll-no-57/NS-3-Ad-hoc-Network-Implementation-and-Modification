...
main(int argc, char* argv[])
{
    ...
    // FIX 1: Increase TX power so nodes can reach each other (~250 m range)
    ...
    std::cout << "[2] Wi-Fi ad-hoc devices installed (TxPower=20 dBm, range ~250m).\n";


    // 3. Mobility — REDUCED speed (1-5 m/s), smaller 300x300m area
    // FIX 2: Lower speed and area so nodes stay within Wi-Fi range
    ...
    mobility.SetMobilityModel("ns3::GaussMarkovMobilityModel",
                              "Bounds",       BoxValue(Box(0, 300, 0, 300, 0, 0)),
    ...
    std::cout << "[3] Mobility installed (Gauss-Markov, 300x300m, 1-5 m/s).\n";
    ...
    aodv.Set("MaxVelocity",   DoubleValue(5.0));   // FIX 3: match new max speed
    ...
    std::cout << "    Alpha=0.5, Beta=0.3, Gamma=0.2, MaxVelocity=5 m/s\n";


    // 6. Applications — multiple flows
    uint16_t port = 9;
    ...
    // 8. NetAnim — must be created BEFORE Simulator::Run()
    AnimationInterface anim("ff-aodv-animation.xml");
    anim.SetMaxPktsPerTraceFile(500000);

    // FIX 4: Color-code nodes so they're easy to identify in NetAnim
    // Red = senders, Green = receivers, Blue = relay nodes
    anim.UpdateNodeColor(nodes.Get(0),          255, 0,   0);   // sender 1 = Red
    anim.UpdateNodeColor(nodes.Get(1),          255, 0,   0);   // sender 2 = Red
    anim.UpdateNodeColor(nodes.Get(nNodes - 1), 0,   200, 0);   // receiver 1 = Green
    anim.UpdateNodeColor(nodes.Get(nNodes - 2), 0,   200, 0);   // receiver 2 = Green
    for (uint32_t i = 2; i < nNodes - 2; i++)
    {
        anim.UpdateNodeColor(nodes.Get(i), 0, 100, 255);        // relay = Blue
    }
    anim.UpdateNodeSize(0, 3, 3);  // make nodes a bit bigger


    // 9. Run
    std::cout << "\n=== Starting FF-AODV v2 simulation (" << simTime << "s, "
              << nNodes << " nodes, low-mobility 1-5 m/s) ===\n\n";
    ...
    for (uint32_t i = 0; i < nNodes; i++)
    {
        ...
        std::cout << "  Node " << i << ": " << remaining << " / " << initial
                  << " J  (" << (remaining / initial * 100.0) << "% remaining)\n";
    }
    ...
    for (auto& entry : stats)
    {
        ...
        if (entry.second.rxPackets > 0)
        {
            ...
            double throughput  = entry.second.rxBytes * 8.0 / (simTime * 1000.0);
    ...
    std::cout << "  [OK] Simulation ran to completion\n";
    std::cout << "  [OK] Energy sources consumed\n";
    std::cout << "  [OK] Velocity accessible\n";
    std::cout << "  [OK] NetAnim XML generated: ff-aodv-animation.xml\n";
